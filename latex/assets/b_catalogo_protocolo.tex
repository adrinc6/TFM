...
evaluó 71 configuraciones en la pasada de referencia. Esa reducción no es sólo computacional: cada
...
Ese argumento vale para el plano predictivo y no se extiende al de cartera, donde este trabajo sí
recorre un producto cartesiano. La diferencia es doble: las seis variables de cartera generan 1.728
combinaciones --- cinco órdenes de magnitud menos que las predictivas ---, y sobre todo no tocan el
Rank-IC, de modo que su búsqueda no puede inflar la métrica que el documento reporta como resultado
principal. El razonamiento completo está en la
Sección~\ref{sec:seleccion-secuencial} y el diseño del estudio correspondiente en el
Capítulo~\ref{chap:desarrollo-cartera}.

\section{Versiones de catálogo empleadas}

El catálogo es un objeto versionado y la cadena no corrió entera bajo la misma versión. Las dos
primeras pasadas usaron la versión 6 y la tercera la versión 7. El único cambio entre ambas afecta
al orden de preferencia por simplicidad de \artefacto{execution\_lag\_days}, que pasó a preferir el
retardo mayor. La corrección se introdujo al detectar que el orden anterior favorecía
sistemáticamente, en caso de empate técnico, la alternativa con más riesgo de anticipación. Como
sólo interviene cuando la evidencia no distingue entre candidatos, no altera ninguna medición de
Rank-IC, pero se declara porque afecta a la comparabilidad estricta entre pasadas.
