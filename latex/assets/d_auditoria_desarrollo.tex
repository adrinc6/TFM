...
Cada uno de estos cambios rompe la comparabilidad con los resultados anteriores a él, y por eso la
evidencia que sostiene este trabajo procede de una única cadena ejecutada bajo el catálogo ya
estabilizado: mezclar mediciones de antes y de después de una corrección sería comparar reglas
distintas.

El número de contratos de prueba pasó de 15 a 75 a lo largo del desarrollo, y casi todos los
añadidos proceden de un defecto concreto: cada corrección de esta lista dejó tras de sí una prueba
que impide su reaparición. Esa es, probablemente, la parte más reutilizable del trabajo.
...
Este historial de correcciones plantea una pregunta legítima, y conviene contestarla con el
artefacto delante: ¿corrió toda la cadena bajo el mismo catálogo, o alguna de las correcciones que
acaba de describirse cae en medio? Las cuatro instantáneas ---la de las tres pasadas y la del
Portfolio Study--- registran la misma versión y el mismo hash de catálogo, de modo que ninguna
corrección del desarrollo atraviesa la cadena adoptada y \textbf{las pasadas son estrictamente
comparables entre sí}. No es una valoración del autor: se comprueba abriendo los cuatro
\artefacto{catalog\_snapshot.json} y contrastando su campo de hash.

Que la comprobación salga limpia no era el desenlace por defecto. Los defectos de este anexo se
corrigieron a lo largo de varias semanas y varios de ellos cambiaron el catálogo; que la cadena
final quepa entera dentro de una sola versión es consecuencia de haberla relanzado desde cero
después de la última corrección, no de que el catálogo no se moviera.

Lo que sí queda declarado es el alcance de esa garantía: cubre la comparabilidad \emph{entre las
pasadas de la cadena}, no la comparabilidad con estudios anteriores del desarrollo, que corrieron
bajo catálogos distintos y cuyas cifras no aparecen en este documento por esa misma razón.
