...
\paragraph{Dos carteras, no una.} Conviven la cartera que el catálogo recomienda, empleada como
punto de partida y referencia de contraste, y la que la rejilla eligió, que es la que el documento
adopta. Producen resultados de \emph{signo opuesto} en la era reservada, y ése es el hallazgo del
capítulo. Toda tabla declara con cuál se calculó.
...
\begin{table}[H]
  ...
  \caption*{Fuente: \artefacto{module/evaluation/profiles.py}, constante
  \texttt{PROFILE\_WEIGHTS}. Abreviaturas: \texttt{qual} = quality, \texttt{val} = value,
  \texttt{grow} = growth, \texttt{mom} = momentum. El conjunto se diseñó para cubrir arquetipos
  reconocibles con solapamiento mínimo: \texttt{momentum} y \texttt{contrarian} son opuestos por
  construcción.}
\end{table}
