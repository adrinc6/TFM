...
\section{De los factores al aprendizaje automático}

\subsection{Las regularidades que orientan el diseño}

Los retornos esperados de una acción no son independientes de sus características observables. Sobre
esa idea se construyó la literatura de factores, que documentó cómo el tamaño, el valor, el momentum
y la calidad de una empresa quedaban asociados a diferencias sistemáticas de rentabilidad (Banz,
1981; Fama y French, 1992; Jegadeesh y Titman, 1993; Novy-Marx, 2013). De ahí salen las cinco
familias en que este trabajo reparte sus variables ---calidad, valor, crecimiento, tendencia y
riesgo---, y de ahí salen también sus baselines: una fórmula determinista de un factor, calculada
sobre el mismo panel, es el listón mínimo que un sistema aprendido debe superar.
...
más aún después de publicarse: McLean y Pontiff (2016) y Hou et al. (2020) reevaluaron cientos de
predictores con criterios homogéneos y encontraron que muchos dejaban de superar umbrales exigentes
o dependían de empresas muy pequeñas. La implicación no es que toda
...
Ninguna de esas primas, aplicada como fórmula fija sobre el panel de este trabajo, llega a ordenar
gran cosa: el Capítulo~\ref{chap:resultados} lo mide al comparar el sistema con esos mismos
\textit{baselines}. No invalida la literatura ---se documentaron sobre universos, periodos y
horizontes distintos---, pero sí acota qué cabe esperar aquí de una regla sin aprendizaje.
...
\subsection{Qué añade el aprendizaje automático y qué arriesga}
...
De esa literatura este trabajo retiene dos ideas y descarta el resto: que la no linealidad puede
importar, y que añadir predictores no garantiza añadir información. La segunda es la que gobierna el
diseño, porque justifica podar variables en vez de acumularlas.
...
A esas cuatro líneas se añade una quinta que no es metodológica sino estructural: la aritmética de
la gestión activa de Sharpe (1991), enunciada en la Sección~\ref{sec:suma-cero-intro}. Su
consecuencia para la literatura revisada es que un trabajo que reporte alfa persistente sobre un
índice amplio está afirmando, implícitamente, que ese alfa lo pagó otro participante. Es la razón de
fondo por la que la replicación encuentra tanta caída fuera de muestra: una anomalía publicada deja
de ser una ventaja informativa en cuanto el resto del mercado puede explotarla.

\section{Sesgos que condicionan cualquier evidencia}
...
alertan del gran número de factores propuestos y de la necesidad de elevar por ello el estándar de
evidencia. De ahí sale la corrección que este trabajo emplea, el Deflated Sharpe Ratio de Bailey y
López de Prado (2014), que descuenta del resultado cuántas configuraciones hubo que probar para
encontrarlo. Ninguna corrección de este tipo reconstruye, sin embargo, una hipótesis verdaderamente
previa después de observar los datos, y por eso el catálogo cerrado, el registro de decisiones y la
era reservada cumplen una función que ningún $p$-valor sustituye.
...
\section{Medir la señal: Rank-IC, amplitud y transferencia}

\subsection{Por qué la métrica es una correlación de rangos}
...
\subsection{De la señal a la cartera: la ley fundamental}

La literatura de gestión activa distingue la calidad de una señal de la capacidad de aprovecharla, y lo resume en una relación clásica ---la ley fundamental de Grinold (1989)--- que
...
La distinción entre calidad de señal e implementación que formula López de Prado (2018) es la que
este trabajo adopta como orden de trabajo: primero la señal, después la cartera, y nunca al revés.
