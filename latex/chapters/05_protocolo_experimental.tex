...
La primera pasada lo dejó claro. Su ganador se apartaba en nueve variables del punto de
...
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[
      >=Latex, font=\footnotesize, align=center, node distance=0mm and 7mm,
      pas/.style={draw,rounded corners,fill=blue!9,minimum width=23mm,minimum height=12mm},
      ini/.style={draw,rounded corners,fill=gray!14,minimum width=20mm,minimum height=12mm},
      fin/.style={draw,thick,rounded corners,fill=green!12,minimum width=19mm,minimum height=12mm},
      etiq/.style={font=\scriptsize,text=black!65,inner sep=1pt}]
    ...
    \draw[->,thick] (b)--(p1);
    \node[etiq] (e1) at ($(b.east)!0.5!(p1.west)+(0,-11mm)$) {9 cambios};
    \draw[->,thick] (p1)--(p2);
    \node[etiq] (e2) at ($(p1.east)!0.5!(p2.west)+(0,-11mm)$) {2 cambios};
    \draw[->,thick] (p2)--(p3);
    \node[etiq] (e3) at ($(p2.east)!0.5!(p3.west)+(0,-11mm)$) {2 cambios};
    \draw[->,thick] (p3)--(w);
    \draw[black!35,line width=0.3pt] (e1.north) -- ($(b.east)!0.5!(p1.west)+(0,-1.5mm)$);
    \draw[black!35,line width=0.3pt] (e2.north) -- ($(p1.east)!0.5!(p2.west)+(0,-1.5mm)$);
    \draw[black!35,line width=0.3pt] (e3.north) -- ($(p2.east)!0.5!(p3.west)+(0,-1.5mm)$);
    \node[below=14mm of p2,font=\scriptsize,text=black!70,align=center]
...
La primera es ganar claramente: su diferencia media es favorable \emph{y} además gana en
...
La segunda es ganar por no ser peor. Cuando la alternativa es más simple o más
...
tras cada paso ---que apenas se mueve, por lo que se explica más abajo--- sino la
...
alternativas, y el resultado merece decirse sin rodeos: el sistema no debe su capacidad
...
El tercer grupo son las cuatro decisiones que la última columna marca como empate técnico:
...
El dato que resume la pasada es que dieciséis de las dieciocho decisiones confirmaron su
...
Dos filas merecen lectura aparte porque en ellas el ganador no es el valor que mejor midió.
...
Ese argumento vale para el plano predictivo por dos motivos. El primero es estadístico: la
...
Ninguno de los dos se aplica al plano de la cartera, y de ahí que allí el trabajo haga lo
...
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[
      >=Latex, font=\footnotesize, align=center, node distance=6mm,
      paso/.style={draw,rounded corners,fill=blue!9,minimum width=52mm,minimum height=9mm},
      dec/.style={draw,rounded corners,fill=orange!14,minimum width=52mm,minimum height=10mm},
      fin/.style={draw,thick,rounded corners,fill=gray!16,minimum width=52mm,minimum height=9mm},
      si/.style={font=\scriptsize,text=black!75,right,inner sep=2pt},
      no/.style={font=\scriptsize,text=black!60,above,inner sep=2pt}]
    \node[paso]              (s) {Se actualiza la señal y el alfa esperado};
    \node[dec, below=of s]   (h) {¿Cumplió la permanencia mínima?};
    \node[dec, below=of h]   (v) {¿Vender mejora tras pagar costes?};
    \node[paso,below=of v]   (d) {Se elige destino: otra acción o efectivo};
    \node[paso,below=of d]   (r) {Se corrige el peso sólo si la deriva compensa};
    \node[fin, below=of r]   (f) {Cartera resultante};
    \draw[->] (s)--(h);
    \draw[->] (h)--node[si]{sí}(v);
    \draw[->] (v)--node[si]{sí}(d);
    \draw[->] (d)--(r);
    \draw[->] (r)--(f);
    % Las dos salidas «no» bajan por el margen derecho: ninguna línea cruza una caja.
    \draw[->,dashed] (h.east) -- node[no]{no} ($(h.east)+(13mm,0)$) |- (f.east);
    \draw[->,dashed] (v.east) -- node[no]{no} ($(v.east)+(8mm,0)$)  |- (f.east);
  \end{tikzpicture}
  \caption{Orden en que la cartera decide, de arriba abajo. El tiempo mínimo en cartera se comprueba
  antes que la economía de la venta, y ninguna posición se vende si no hay un destino mejor después
  de costes. Las dos flechas discontinuas son las salidas que conservan la posición sin tocarla.}
...
recortada en 2024. La distinción es fina y decisiva: no se trata de calcular el resultado
...
consistencia, no el alfa bruto. Una cartera que bate al índice un 2\,\% todos los años
...
interactúan, de modo que recorrerlas de una en una daría una respuesta que depende del
