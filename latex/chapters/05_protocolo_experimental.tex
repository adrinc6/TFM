...
lo dejó claro en cifras: su ganador se apartaba en \textbf{nueve variables} del baseline recomendado
...
suficiente para desplazarlo. La primera modificó nueve variables respecto de su punto de partida; la
segunda, dos (\artefacto{max\_features\_per\_agent} y \artefacto{lgbm\_min\_child\_samples}); la
tercera, otras dos (\artefacto{lgbm\_learning\_rate} y \artefacto{market\_regime\_feature}). El
procedimiento se queda sin cambios
...
La Figura~\ref{fig:protocolo} separa gráficamente las dos mitades del protocolo, y esa separación es
la garantía que sostiene todo lo demás: sólo las cuatro fases superiores pueden mover al ganador, y
todo lo económico ocurre a la derecha de la señal ya congelada.
...
\begin{table}[H]
  ...
  que promedia la diferencia cohorte a cohorte: en \artefacto{meta\_method} el coste es $-0{,}0071$ y
  la ventaja pareada $+0{,}00699$.}
\end{table}
...
menos: los hiperparámetros de LightGBM (profundidad y número de árboles, ambos exactamente
$-0{,}0050$; tasa de aprendizaje y mínimo de observaciones por hoja, por debajo de $0{,}0005$), el
lag de ejecución, la winsorización, el número máximo de variables por agente y la regla de
ponderación de variables. Ahí la evidencia apenas distingue entre alternativas, y conviene decirlo
abiertamente: el sistema no debe su capacidad predictiva al ajuste fino del modelo. Entre los dos
extremos quedan tres decisiones intermedias ---la cadencia de snapshots ($-0{,}0176$), la ventana de
entrenamiento ($-0{,}0120$) y la variante del meta-agente ($-0{,}0071$)---, y las tres también
confirmaron el valor que ya traían.

El tercer grupo son las \textbf{cuatro} decisiones que la última columna marca como empate técnico:
\artefacto{execution\_lag\_days}, \artefacto{winsorization}, \artefacto{market\_regime\_feature} y
\artefacto{lgbm\_learning\_rate}. Ninguna de las cuatro debe presentarse como hallazgo empírico, y
por eso figuran con su regla al lado.

Dos filas merecen además una lectura aparte, porque son aquellas en las que \textbf{el ganador no es
el valor que mejor midió}. En \artefacto{winsorization} la alternativa de $0{,}025$ obtuvo $0{,}1068$
frente al $0{,}1063$ del incumbente ---coste $+0{,}0005$---, la diferencia quedó dentro del umbral de
empate técnico y se impuso la preferencia declarada por la opción más simple, que aquí es no
winsorizar. En \artefacto{feature\_weighting\_mode} ocurre algo distinto y más instructivo:
\texttt{model\_native} medía $0{,}1065$ frente a $0{,}1063$ ---coste $+0{,}0002$--- y su ventaja
pareada era incluso positiva ($+0{,}0017$), pero ganaba en el $49{,}6$\,\% de las cohortes, es decir,
en menos de la mitad. No superó por tanto la puerta de dominancia y el incumbente se conservó. Es el
ejemplo más claro de por qué la regla mira la serie de diferencias emparejadas y no el promedio
agregado: una media favorable construida sobre menos de la mitad de las fechas no es evidencia de
superioridad.
...
tipo de búsquedas: \textbf{dieciséis de las dieciocho decisiones confirmaron su valor de partida}, y
las dos que lo cambiaron lo hicieron por márgenes mínimos --- \artefacto{market\_regime\_feature}
por no inferioridad, sin demostrar ser mejor, y \artefacto{lgbm\_learning\_rate} con una ventaja
media de tres diezmilésimas de Rank-IC. Entre las confirmadas, la más relevante para la arquitectura
es \artefacto{meta\_method}, donde la variante libre obtuvo 0,1067 frente a 0,0996 de la acotada;
también ahí el procedimiento conservó el valor que ya traía, porque la variante libre venía heredada
de la primera pasada de la cadena.
...
persiguiendo diferencias dentro del ruido. Que dieciséis de las dieciocho decisiones terminen en «no cambiar»
...
Ninguno de los dos motivos se aplica al plano de la \textbf{cartera}, y de ahí que allí el trabajo
haga lo contrario. Las reglas de cartera no tocan el modelo ni modifican el Rank-IC: operan sobre
una señal ya congelada, así que optimizarlas no puede inflar la métrica predictiva que el documento
reporta. Y su elección no necesita narrarse como una escalera de decisiones porque no lo es, según
se argumenta al presentar el Portfolio Study en la Sección~\ref{sec:portfolio-study}. Queda un
riesgo real, la multiplicidad de comparaciones, que no se disimula y se declara en el
Capítulo~\ref{chap:limitaciones}.

\section{Ejemplo de comparación pareada}

\begin{ejemplo}[Mecánica de la comparación, sobre un caso real]
Así se decidió la cadencia mensual frente al baseline trimestral en la primera pasada de la cadena.
En cada fecha se resta el Rank-IC del incumbente al del candidato y se re-muestrean bloques de doce
cohortes para preservar la dependencia inducida por el horizonte anual. La cadencia mensual obtuvo
una ventaja media de $+0{,}0136$ sobre 40 fechas emparejadas, siendo mejor en el $60{,}0$\,\% de
ellas, con un intervalo \textit{bootstrap} al 90\,\% de $[-0{,}0000;\,0{,}0327]$.
\end{ejemplo}

El caso muestra que las dos puertas no son la misma. La cadencia mensual desplazó a la trimestral por
\emph{dominancia} ---ventaja media positiva y mejor en más de la mitad de las fechas---, no por el
intervalo, cuyo límite inferior rozaba cero. Un procedimiento que exigiera además un intervalo
estrictamente positivo habría conservado la cadencia trimestral, y con ella habría perdido las 117
cohortes mensuales sobre las que descansa el resto del trabajo.
...
universo. El horizonte de seis meses ilustra un rechazo medido: en la pasada de referencia su ventaja pareada
fue $-0{,}0375$ y el intervalo $[-0{,}0777;\,-0{,}0124]$, íntegramente negativo. Mostrar este resultado evita presentar la selección como una
...
El primer paso es que la cartera necesita una magnitud económica, no sólo un percentil. Sobre
cohortes cerradas se estima una relación entre ventiles de \texttt{meta\_rank} y retorno excedente
anualizado. Se usan veinte grupos para que cada tramo conserve suficientes acciones y para no
confundir una ordenación ordinal con una precisión individual inexistente.

\subsection*{Calibración causal y estados de respaldo}

Para cada fecha de inversión $t$, el calibrador sólo recibe cohortes cuyo retorno a doce meses ya
terminó antes de $t$. Primero calcula, para cada ventila $q$, el exceso medio observado
$\bar r^{\,e}_{q,t}$ y ajusta una función monótona que traduce rango en alfa esperado:
\[
 \widehat{\alpha}_{i,t}=g_t\!\left(\operatorname{rank}_{i,t}\right).
\]
La monotonía no obliga a que el mercado pague una cantidad concreta; impone únicamente la
coherencia mínima de que una acción mejor ordenada no reciba menos alfa esperado que otra peor
ordenada. La magnitud procede de la historia cerrada y se expresa en puntos básicos anuales, de
modo que los umbrales operativos mantienen el mismo significado cuando cambia el horizonte.

La estimación más local usa cohortes recientes del mismo horizonte. Si no existe historia
suficiente o la relación no puede estimarse de forma estable, se amplía primero a dieciséis
trimestres; si sigue sin ser viable, se utiliza todo el histórico cerrado. Sólo cuando tampoco esa
información existe se activa una salvaguarda determinista. Los cuatro estados son
\texttt{horizon}, \texttt{era}, \texttt{none} y \texttt{fallback}; cada snapshot registra cuál se
empleó. Esta cascada evita dos errores opuestos: fingir precisión local con muy pocas observaciones
y mezclar silenciosamente un supuesto con una estimación.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/f05_calibracion_alfa.png}
  \caption{Trazabilidad de la calibración. Arriba, fuente empleada en cada snapshot; abajo,
  relación entre ventila del ranking y alfa anual esperado para cortes representativos. La franja
  reservada se muestra para verificar aplicación, nunca para elegir el calibrador.}
  \label{fig:calibracion-alfa}
\end{figure}
...
En selección, 72 de los 117 snapshots usan la estimación del mismo horizonte, 24 amplían a la era,
15 quedan sin calibración empírica suficiente y seis activan la salvaguarda de arranque. En la
reserva, los 18 snapshots aplican la estimación de horizonte obtenida con historia anterior. Este
recuento importa más que una curva elegida como ejemplo: hace visible cuándo el sistema aprendió
una escala económica y cuándo todavía operaba con una regla de seguridad.

La calibración no crea información predictiva adicional. Dos acciones conservan el orden que les
dio el meta-agente; $g_t$ sólo cuantifica la distancia económica necesaria para comparar una
candidata con costes, efectivo y una posición existente. Por eso sus parámetros no participan en
la selección por Rank-IC y su calidad se juzga después mediante turnover, costes y estabilidad de
la cartera.
...
Las seis variables no se aplican como filtros independientes. En cada rebalanceo existe una
precedencia explícita que impide contradicciones: primero se actualizan señales y restricciones de
tenencia; después se evalúan ventas económicamente elegibles; a continuación se comparan destinos
para el capital liberado; finalmente se ajustan tamaños sólo si la deriva justifica pagar el coste.

\begin{figure}[H]
  \centering
  \begin{tikzpicture}[node distance=8mm and 10mm,>=Latex,every node/.style={font=\small,align=center}]
    \node[draw,rounded corners,fill=blue!8,minimum width=28mm,minimum height=10mm] (s) {Señal y alfa\\actualizados};
    \node[draw,rounded corners,fill=orange!12,right=of s,minimum width=31mm,minimum height=10mm] (h) {¿Tenencia mínima\\cumplida?};
    \node[draw,rounded corners,fill=yellow!15,right=of h,minimum width=34mm,minimum height=10mm] (v) {¿Vender mejora\\después de costes?};
    \node[draw,rounded corners,fill=teal!10,below=of v,minimum width=34mm,minimum height=10mm] (d) {Mejor destino:\\acción o efectivo};
    \node[draw,rounded corners,fill=green!10,left=of d,minimum width=31mm,minimum height=10mm] (r) {Ajuste por deriva\\si compensa};
    \node[draw,rounded corners,fill=gray!12,left=of r,minimum width=28mm,minimum height=10mm] (p) {Cartera\\resultante};
    \draw[->,thick] (s)--(h);
    \draw[->,thick] (h)--node[above]{sí}(v);
    \draw[->,thick] (v)--node[right]{sí}(d);
    \draw[->,thick] (d)--(r);
    \draw[->,thick] (r)--(p);
    \draw[->,thick,dashed] (h.south) |- node[pos=0.25,left]{no} (p.north);
    \draw[->,thick,dashed] (v.north) -- ++(0,0.55) -| node[pos=0.25,above]{no} (p.north);
  \end{tikzpicture}
  \caption{Orden de decisión de la cartera. La permanencia por tiempo se comprueba antes que la
  economía de la venta; una venta sólo ocurre si existe un destino netamente mejor.}
  \label{fig:precedencia-cartera}
\end{figure}

Esta jerarquía aclara una diferencia importante: el mínimo de tenencia puede impedir cualquier
venta por tiempo, mientras que el suelo de cobertura o un alfa negativo sólo hacen que una venta
sea candidata. Incluso entonces debe existir un destino superior después de costes. SPY no puede
ser ese destino porque sólo actúa como benchmark; el efectivo sí puede serlo, pero únicamente
dentro del tope \texttt{max\_cash\_weight}.
...
Las entradas nuevas incorporan histéresis y el efectivo rinde 0\%. Bajo la política de efectivo, una
plaza puede permanecer en caja sólo si respeta el tope declarado
...
por separado, sino \texttt{target\_\allowbreak size} y \texttt{max\_cash\_\allowbreak weight} conjuntamente: el peso
máximo por posición depende de cuántas posiciones caben y de cuánta caja se tolera. Y
\texttt{coverage\_percentile\_\allowbreak floor} hace cosas \emph{opuestas} según el tope de efectivo: con
