...
Tabla~\ref{tab:limitaciones} reúne las que condicionan de verdad la lectura del trabajo. No es un
inventario exhaustivo de todo lo mejorable: recoge aquellas cuya magnitud cambia lo que puede
afirmarse, y deja fuera las salvedades genéricas que no mueven ninguna conclusión.
...
La multiplicidad se agravó dos veces y a propósito: al encadenar tres Model Studies y al
...
El predominio del agente de riesgo queda matizado y agravado a la vez por su atribución
...
La concentración en pocos nombres no la delata ninguna métrica agregada: de las 42 acciones
...
Y la que más condiciona las conclusiones: el orden predictivo y el pago económico no se mueven en
la misma dirección. El Rank-IC es mayor en la era más reciente, mientras que el
...
La causa está medida en el Capítulo~\ref{chap:datos} y no es la mortalidad empresarial sino
...
La dirección es optimista, no conservadora. Las empresas que sí llegan al panel sobreviven
...
Queda por declarar el límite de lo demostrable. Lo medido es la composición del universo,
...
Dos límites acotan hasta dónde viaja este resultado. El primero es de escala: el
Capítulo~\ref{chap:resultados-economicos} sitúa el techo de ejecución en unas pocas decenas de
millones, de modo que las conclusiones económicas se refieren a un patrimonio individual y no a una
cartera de mayor tamaño. El segundo es de universo: al trabajar sobre grandes capitalizaciones
estadounidenses, nada de lo medido aquí puede extrapolarse a empresas pequeñas, otros países o
regímenes de liquidez distintos.

Cada limitación acota una afirmación diferente, y conviene no mezclarlas: la multiplicidad limita el
Sharpe, la cobertura limita la generalización y la cartera limita la transferencia de señal. Ninguna
permite ignorar los placebos, la permutación o la estabilidad por eras.
...
\begin{table}[htbp]
  ...
  \input{tables/t08_ventas_prematuras.tex}
...
Apple y la mantuvo cuarenta y cinco meses seguidos, y esa es la cara favorable. La desfavorable es que las
...
\subsection{Dos decisiones que condicionan las cifras}

Dos limitaciones no proceden del diseño sino de decisiones tomadas durante el desarrollo. Merecen
...
La primera es la salvaguarda de la curva de alfa: cuando ninguna ventana de la cascada produce una
calibración creciente, una recta fija declarada de antemano sustituye a la calibración empírica. Se
dispara precisamente cuando la evidencia indica que no hay señal que calibrar, así que allí la
expectativa de alfa no procede de los datos sino de un supuesto de diseño. El Anexo~\ref{anexo:desarrollo}
recoge la disyuntiva que la motivó y por qué se prefirió a no tener alfa esperada en absoluto.

La segunda afectaba a las variables de cartera, que en versiones anteriores de este trabajo tomaban
