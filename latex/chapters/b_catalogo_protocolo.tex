...
\artefacto{decisions.json}: catorce decisiones se resolvieron por \texttt{robust\_rank\_ic} y cuatro
por \texttt{tie\_simplicity}.

Esas dieciocho decisiones no cubren las veintiuna variables predictivas del catálogo, y la diferencia
conviene declararla. Tres --- \artefacto{model\_family}, \artefacto{meta\_history\_quarters} y
\artefacto{meta\_recency\_weighting} --- se ejecutaron en modo \texttt{fixed} en la pasada de
referencia, es decir, con un único valor alcanzable de su rejilla, de modo que no había comparación
que hacer. Sus valores finales son los que la tabla recoge, pero provienen de una decisión declarada
de antemano y no de una medición pareada, exactamente igual que los cuatro empates técnicos.
...
seis de las doce variables de cartera y cambió \textbf{cuatro} de ellas ---
\artefacto{max\_cash\_weight} de 0,25 a 0,10; \artefacto{minimum\_holding\_period} de
\texttt{half\_horizon} a \texttt{full\_horizon}; \path{coverage_percentile_floor} de 60 a 0; y
\artefacto{rebalance\_drift\_tolerance} de 0,25 a 0,40 ---, mientras que \artefacto{target\_size} y
\artefacto{sizing\_mode} coincidieron con los de partida. La configuración adoptada se presenta en la
Sección~\ref{sec:cartera-ganadora}. Se tabula aquí el ganador del Model Study porque este anexo
documenta el catálogo del protocolo predictivo, no la gestión de cartera.

\paragraph{Por qué seis de las doce.} Las otras seis variables de cartera ---
\path{commission_bps}, \path{slippage_bps}, \path{exit_expected_alpha_bps},
\path{rotation_edge_bps}, \path{price_only_strictness_multiplier} y
\path{price_only_sell_only} --- declaran rejilla pero no entran en el cartesiano, y la razón
...
cartesiano de las treinta y tres rejillas asciende a $4{,}6 \times 10^{14}$ combinaciones; incluso
...
La tabla se genera directamente desde \texttt{feature\_catalog.json}; no es una transcripción
manual. Por ello cumple dos funciones. Para el lector, hace explícito qué significan nombres
abreviados como \texttt{gap\_21d} o \texttt{range\_63d}. Para la auditoría, permite comprobar que
una variable citada en atribución existe en el catálogo y que ningún predictor improvisado aparece
sólo en el resultado. Los diagnósticos auxiliares que no entrenan modelos se conservan fuera de
esta lista para no confundir control de calidad con información predictiva.
