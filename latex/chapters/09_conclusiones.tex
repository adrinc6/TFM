...
Un contraste, sin embargo, no se supera, y es el que corrige por haber buscado: el Deflated Sharpe
se queda en \ProbabilidadDSR{}, por debajo del umbral de 0,95. La consecuencia hay que enunciarla
con precisión, porque no anula lo anterior. Que la ordenación existe lo sostienen la permutación y
el bootstrap, que no dependen del número de configuraciones ensayadas; lo que la multiplicidad
impide es afirmar una \emph{magnitud} ajustada por todo lo que se probó para encontrarla. La
capacidad de ordenar queda establecida; su tamaño exacto, no.
...
\subsection{Sensibilidad a la construcción de cartera}
...
\subsection{Comportamiento en la era reservada}
...
De ahí se sigue la tesis del trabajo: ninguna afirmación práctica sobre un sistema predictivo es
interpretable sin declarar con qué cartera se midió. La frase no convierte a la
...
la cartera pasó por \NumAccionesCartera{} acciones, pero una sola aporta \ContribucionAAPL{} de
contribución neta acumulada. Esa concentración reduce el número de experiencias independientes que
sostienen el resultado, y es la razón de que la conclusión económica deba ser más prudente que la
predictiva.

\section{Trabajo futuro y respuesta final}
...
\begin{enumerate}[leftmargin=*,itemsep=0.5em]
  ...
  \textit{slippage} constantes. El techo medido aquí ---unas pocas decenas de millones, el ámbito de
  un patrimonio individual--- procede de la participación sobre el volumen habitual, no de un modelo
  de impacto, y precisarlo exigiría lo segundo.
\end{enumerate}
