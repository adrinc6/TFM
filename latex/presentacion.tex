% Defensa del TFM: 20 diapositivas totales, guion objetivo 18:35.
...
% 12 · 0:50
\setacto{teal}
\begin{frame}{La cartera pasa a ser el segundo objetivo}
  \vspace{0.5cm}
  \begin{center}
    {\fontsize{24}{28}\selectfont\color{navy}\textbf{Una ordenación no se puede comprar entera.}}
  \end{center}
  \vspace{0.7cm}
  \begin{columns}[t,onlytextwidth]
    \begin{column}{0.30\textwidth}\cifra{cuántas}{posiciones}\end{column}
    \begin{column}{0.30\textwidth}\cifra{cuándo}{comprar y vender}\end{column}
    \begin{column}{0.30\textwidth}\cifra{cuánto}{peso y efectivo}\end{column}
  \end{columns}
  \vspace{0.5cm}
  \begin{center}{\large Mismo modelo · misma señal · distintas reglas}\end{center}
\end{frame}
\note{[0:50] La cartera compra ocho o veinte nombres, paga costes y decide cuándo salir. En los
Model Studies esas reglas eran las recomendadas del catálogo, no el objeto de selección. El segundo
objetivo mide si esas decisiones cambian materialmente el resultado.}
...
\begin{frame}{Qué queda demostrado y qué no}
  \begin{columns}[t,onlytextwidth]
    \begin{column}{0.47\textwidth}
      {\color{verde}\Large\textbf{Sí}}\\[0.35cm]
      \begin{itemize}\setlength{\itemsep}{0.45em}
        \item Existe ordenación predictiva favorable y robusta.
        \item Las reglas de cartera alteran materialmente el resultado.
        \item La implementación puede cambiar el signo con señal fija.
      \end{itemize}
    \end{column}
    \begin{column}{0.47\textwidth}
      {\color{rojo}\Large\textbf{No todavía}}\\[0.35cm]
      \begin{itemize}\setlength{\itemsep}{0.45em}
        \item Necesidad de cinco agentes.
        \item Rentabilidad ajustada por multiplicidad.
        \item Generalización económica de la cartera elegida.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
\note{[1:10] El objetivo uno queda apoyado dentro del universo y protocolo, con risk ligeramente por
encima del meta. El objetivo 2a queda demostrado por la dispersión de 1.440 carteras. El 2b sigue
siendo preliminar por selección y tamaño muestral. Esta separación evita vender el resultado más de
lo que permite la evidencia.}

% 19 · 1:15
\setacto{navy}
\begin{frame}{La señal y la cartera deben defenderse por separado}
  \vspace{0.25cm}
  \begin{columns}[t,onlytextwidth]
    \begin{column}{0.30\textwidth}
      {\color{navy}\Large\textbf{1}}\\[0.25cm]
      {\large Ordenación favorable y robusta.}\\[0.3cm]
      {\color{slate}Sin demostrar la necesidad multiagente.}
    \end{column}
    \begin{column}{0.30\textwidth}
    ...
    \begin{column}{0.30\textwidth}
      ...
      {\large La reserva es alentadora.}\\[0.3cm]
      {\color{slate}Aún no confirma generalización.}
    \end{column}
  \end{columns}
  \vspace{0.85cm}
  \begin{center}
...
robustez e implementación y declara qué queda apoyado, qué es preliminar y qué no se demuestra.}

% 20 · 0:20 — total objetivo 18:35
{\setbeamertemplate{footline}{}
\begin{frame}[plain]
  \vspace{2.0cm}
