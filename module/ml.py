...
All four component models (quality, improvement, mispricing, alpha) are trained against genuine
forward-looking targets built from information observable `walk_forward_label_horizon_months`
ahead of each snapshot (see _add_component_targets), leakage-masked identically during walk-forward
training (see _walk_forward_component_scores). None of the four is a restatement of same-day
input features.
"""
    ...
    write_json(
        {
            ...
            "component_target_definitions": {
                "quality_probability": "Forward change in quality_score over the label horizon (improves vs. today).",
                "improvement_probability": "Forward realized_growth vs. today's expected_growth (expectation beaten or missed).",
                "mispricing_probability": "Whether today's valuation discount resolved via a positive forward stock return.",
...
def _add_component_targets(df: pd.DataFrame, prices: pd.DataFrame, benchmark_ticker: str, horizon_months: int) -> pd.DataFrame:
    ...
    All four targets (future_alpha, quality, improvement, mispricing) share the same forward
    horizon and the same walk-forward observability mask (see _walk_forward_component_scores),
    so a single auditable leakage rule covers every component. 12 months is used because it
    reliably spans at least one fresh quarterly fundamental snapshot on top of the monthly price
    cadence; a shorter fundamental-specific horizon would need its own masking/diagnostics for
    no clear benefit at this dataset's cadence.
    """
    prices = prices.copy()
    ...
    feature_cache = _feature_cache(df, ["quality_score", "realized_growth"])

    alphas = []
    quality_targets = []
    improvement_targets = []
    mispricing_targets = []
    log.info("Building forward-looking labels rows=%s horizon_months=%s", len(df), horizon_months)
    for index, row in enumerate(df.itertuples(index=False), start=1):
        ...
        fwd_quality = _forward_feature_value(feature_cache, row.ticker, row.snapshot_date_dt, "quality_score", horizon_months)
        quality_targets.append(
            None if fwd_quality is None else float(min(1.0, max(0.0, (fwd_quality - row.quality_score + 1) / 2)))
        )

        fwd_realized_growth = _forward_feature_value(feature_cache, row.ticker, row.snapshot_date_dt, "realized_growth", horizon_months)
        ...
        if stock_ret is None:
            ...
            gap = row.positive_expectation_gap
            mispricing_targets.append(gap * (1.0 if stock_ret > 0 else 0.0) + (1 - gap) * 0.5)
...
def _walk_forward_component_scores(df: pd.DataFrame, settings: Settings) -> tuple[pd.DataFrame, dict, pd.DataFrame]:
    ...
    log.info("Walk-forward scoring snapshots=%s components=%s", len(dates), len(COMPONENT_TARGETS))
    for date_index, date in enumerate(dates, start=1):
        ...
        if fallback_reasons:
            log.info(
                "Walk-forward snapshot %s/%s %s fallback=%s training_rows=%s training_years=%.2f",
                date_index,
                len(dates),
            ...
            continue
        log.info(
            "Walk-forward snapshot %s/%s %s fitting training_rows=%s training_years=%.2f alpha_labels=%s",
            date_index,
            len(dates),
            pd.Timestamp(date).date().isoformat(),
            len(train),
