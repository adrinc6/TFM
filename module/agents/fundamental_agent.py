...
class FundamentalAgent(BaseAgent):
    ...
    def _prepare(self, X: pd.DataFrame, sector_col: str, fit_mode: bool) -> pd.DataFrame:
        ...
        selected  = list(dict.fromkeys(base_cols + zsec_cols))

        if sector_col in df.columns:
            dummies = pd.get_dummies(df[sector_col], prefix="sector", dtype=float)
            dummies.index = df.index  # garantía explícita de alineación
            if fit_mode:
                self._sector_dummies = list(dummies.columns)
            else:
                for c in self._sector_dummies:
                    if c not in dummies.columns:
                        dummies[c] = 0.0
                dummies = dummies[[c for c in self._sector_dummies if c in dummies.columns]]
            df = pd.concat([df, dummies], axis=1)
            selected += self._sector_dummies
