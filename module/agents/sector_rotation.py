...
class SectorRotationAgent(BaseAgent):
    ...
    def _build_sector_dataset(
        ...
        for (sector, quarter), grp in temp.groupby(["_sector", "_quarter"]):
            ...
            # Macro-context interactions: how sector strength behaves given
            # broad market trend and risk regime in the same snapshot.
            m12 = float(feat.get("momentum_12m", 0.0)) if "momentum_12m" in feat else 0.0
            sp_m12 = float(feat.get("sp500_momentum_12m", 0.0)) if "sp500_momentum_12m" in feat else 0.0
            vix = float(feat.get("vix", 20.0)) if "vix" in feat else 20.0
            feat["sector_vs_market_momentum"] = m12 - sp_m12
            feat["risk_adjusted_sector_momentum"] = m12 / (1.0 + max(vix, 0.0) / 20.0)
    ...
    def _aggregate_ticker_features(
        ...
        # Keep macro-context interactions consistent between train and predict.
        if not agg.empty:
            m12 = agg["momentum_12m"] if "momentum_12m" in agg.columns else pd.Series(0.0, index=agg.index)
            sp_m12 = agg["sp500_momentum_12m"] if "sp500_momentum_12m" in agg.columns else pd.Series(0.0, index=agg.index)
            vix = agg["vix"] if "vix" in agg.columns else pd.Series(20.0, index=agg.index)
            agg["sector_vs_market_momentum"] = pd.to_numeric(m12, errors="coerce").fillna(0.0) - pd.to_numeric(sp_m12, errors="coerce").fillna(0.0)
            agg["risk_adjusted_sector_momentum"] = pd.to_numeric(m12, errors="coerce").fillna(0.0) / (1.0 + pd.to_numeric(vix, errors="coerce").fillna(20.0).clip(lower=0.0) / 20.0)
        return agg
