...
class SentimentAgent(BaseAgent):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series,
        ...
        if not self.has_multiple_classes(y_cl):
            log.warning("[SentimentAgent] Label without enough class variance after cleaning - training skipped.")
            return self
    ...
    def _cv(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        ...
        y = y.reindex(X.index)
        if len(X) < 3:
            log.warning("[SentimentAgent] CV skipped: insufficient samples (%s)", len(X))
            return {"mean_auc": 0.0, "std_auc": 0.0, "mean_acc": 0.0, "mean_f1": 0.0}
        n_splits = min(5, len(X) - 1)
        if n_splits < 2:
            log.warning("[SentimentAgent] CV skipped: invalid n_splits=%s for n=%s", n_splits, len(X))
            return {"mean_auc": 0.0, "std_auc": 0.0, "mean_acc": 0.0, "mean_f1": 0.0}
