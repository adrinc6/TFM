...
def rank_ic_by_year(diagnostics: pd.DataFrame, agent: str = "meta_final") -> pd.DataFrame:
    """Tabla anual del rank-IC de un score (por defecto meta_final): rank-IC medio, fracción
    positiva, nº de cohortes y nº medio de empresas por cohorte.

    Es el diagnóstico temporal que decide si el sistema aprende mejor en ciertos años (más
    cobertura, menos sesgo de supervivencia) y si conviene centrar el proyecto en ese período.
    """
    subset = diagnostics.loc[diagnostics["agent"] == agent].copy()
    if subset.empty:
        return pd.DataFrame(
            columns=["year", "rank_ic_mean", "rank_ic_positive_fraction", "n_cohorts", "avg_names"]
        )
    subset["year"] = pd.to_datetime(subset["prediction_date"]).dt.year
    rows = []
    for year, group in subset.groupby("year"):
        rows.append({
            "year": int(year),
            "rank_ic_mean": float(group["rank_ic"].mean()),
            "rank_ic_positive_fraction": float((group["rank_ic"] > 0).mean()),
            "n_cohorts": int(len(group)),
            "avg_names": float(group["observations"].mean()),
        })
    return pd.DataFrame(rows).sort_values("year").reset_index(drop=True)


def rank_ic_window_summary(
    diagnostics: pd.DataFrame, start_years: tuple[int, ...], agent: str = "meta_final"
) -> pd.DataFrame:
    """Compara ventanas con distinto año de inicio: rank-IC medio, IC bootstrap y nº cohortes.

    Para la Puerta 3: permite ver si concentrarse en años recientes mejora el aprendizaje de
    forma estable (meseta) o solo por episodios puntuales.
    """
    subset = diagnostics.loc[diagnostics["agent"] == agent].copy()
    subset["year"] = pd.to_datetime(subset["prediction_date"]).dt.year
    subset = subset.sort_values("prediction_date")
    rows = []
    for start in start_years:
        window = subset.loc[subset["year"] >= start]
        if window.empty:
            continue
        ci = block_bootstrap_ci(window.set_index("prediction_date")["rank_ic"])
        rows.append({
            "start_year": start,
            "rank_ic_mean": ci["mean"],
            "ci_low": ci["ci_low"],
            "ci_high": ci["ci_high"],
            "positive_fraction": float((window["rank_ic"] > 0).mean()),
            "n_cohorts": int(len(window)),
            "distinguishable_from_zero": bool(ci["ci_low"] > 0),
        })
