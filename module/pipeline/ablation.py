# =============================================================================
# module/pipeline/ablation.py
# Ablation study: evalúa la contribución de cada agente al ensemble.
# =============================================================================
"""
Responsabilidades:
  - run_ablation_study: para cada agente, entrena el meta-learner sin ese
    agente y compara el AUC vs el modelo completo. Guarda resultados en JSON
    y CSV para incluir en la memoria del TFM.

El ablation study utiliza los mismos datos de un fold dado (df_train_norm,
y_train, df_test_norm, y_test) que ya ha sido preparado por el evaluador.
Solo re-entrena el meta-learner, los agentes base ya están entrenados.

Metodología:
  Para cada agente A en {fundamental, valuation, momentum, bear}:
    1. Excluir la columna <A>_score del DataFrame de entrada al meta-learner.
    2. Re-entrenar el meta-learner sin esa feature.
    3. Evaluar AUC en test.
  Comparar contra el modelo completo (baseline).
  La diferencia de AUC mide la contribución marginal del agente.
"""
import json
import logging
import numpy as np
...
# Agentes base cuya contribución se evalúa
...
def run_ablation_study(
    ...
    """
    Evalúa la contribución marginal de cada agente al meta-learner mediante
    ablation: re-entrena el MetaLearner eliminando un agente cada vez y
    compara el AUC resultante con el modelo completo.

    Args:
        df_test_scored: DataFrame de test con columnas *_score y final_score
            (salida de train_fold).
        y_test: Labels binarios de test.
        df_train_norm: DataFrame de train normalizado (con columnas *_score OOF).
        y_train: Labels binarios de train.
        agents_results_dir: Directorio donde guardar resultados.
        fold_id: Número de fold para nombrar el fichero.
        random_seed: Semilla de reproducibilidad.

    Returns:
        Diccionario con AUC baseline y AUC de cada variante ablada, más
        la contribución marginal (baseline_auc - ablated_auc) de cada agente.
    """
    try:
    ...
    # ── Preparar features del meta-learner (solo scores, sin macro ni sector)
    # Usamos una LR sencilla para el ablation (mismo espíritu que MetaLearner pero
    # más rápido; no re-entrenamos GBM para mantener el ablation manejable).
    ...
    def _auc(X_tr, y_tr, X_te, y_te):
        """Entrena LR y devuelve AUC en test. Devuelve NaN si solo hay una clase."""
        pipe = Pipeline([
    ...
    # ── Baseline: todos los agentes
    baseline_auc = _auc(X_train, y_tr, X_test, y_te)

    # ── Ablation: quitar un agente a la vez
    ablation_results = {}
    for ag in ABLATION_AGENTS:
        ...
        log.info(
            ...
            f"AUC={auc_ab:.4f} (baseline={baseline_auc:.4f}, Δ={contribution:+.4f})"
        )
...
def summarize_ablation(
    ...
    """
    Agrega los resultados de ablation de todos los folds y genera:
      - ablation_summary.json: contribución media por agente
      - ablation_summary.csv: tabla legible (una fila por agente)

    Args:
        ablation_results: Lista de resultados de run_ablation_study() por fold.
        agents_results_dir: Directorio donde guardar los ficheros.
    """
    if not ablation_results:
        return

    # Recopilar contribuciones por agente a lo largo de los folds
    ...
    if rows:
        ...
        for _, row in df.iterrows():
            log.info(
                f"  {row['agent']:<15}  ΔAuC medio={row['mean_contribution']:+.4f} "
