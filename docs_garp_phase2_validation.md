...
La comparación de investigación debe ser:

- cartera viva GARP vs SPY desde `PORTFOLIO_START_DATE` hasta `PORTFOLIO_END_DATE`;
- cartera viva GARP vs sector/benchmark cuando existan datos comparables;
- folds walk-forward con `GARP_TARGET_HORIZON_MONTHS` como validación de etiqueta, no como regla de permanencia;
...
## 15. Live Portfolio Evolution

La unidad principal de análisis operativo pasa a ser cartera + evolución temporal + decisiones de gestión. `portfolio_evolution` reutiliza el Thesis Engine y los snapshots ya existentes para simular revisiones periódicas (`PORTFOLIO_REVIEW_FREQUENCY`: `M`, `2M`, `Q`) sin nuevos agentes ni modelos.

Outputs: `portfolio_evolution.csv`, `portfolio_transactions.csv`, `portfolio_monthly_holdings.csv`, `portfolio_decision_log.csv`, `portfolio_monthly_summary.json`, y páginas viewer `portfolio_evolution.html`, `portfolio_vs_benchmark.html`, `allocation_dashboard.html`, `alerts.html`, `snapshot_compare_<ticker>.html` y `thesis_radar_<ticker>.html`.

## 16. Final alignment: live GARP portfolio manager

Portfolio Evolution 2.0 adds position memory and rotation discipline. Existing holdings carry entry date, original snapshot, original thesis, original opportunity type, original scores, months in portfolio, months thesis intact, persistence score, improvement/deterioration counts and cumulative return when prices exist. Rotation is not triggered by marginal score differences; replacements require material score/conviction advantage plus weakening, overvaluation, high exit score or clear thesis risk.

Additional outputs: `portfolio_turnover.csv`, professional `portfolio_decision_log.csv` fields, and viewer pages `portfolio_lifecycle.html`, `portfolio_turnover.html`, `decision_log.html`, `thesis_persistence.html`, `hold_winners.html` and `portfolio_timeline.html`.
