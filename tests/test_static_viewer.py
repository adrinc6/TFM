...
def test_static_viewer_generates_dashboard_pages_and_position_detail(tmp_path):
    ...
    pd.DataFrame(
        {
            "date": ["2024-03-31"],
            "avg_thesis_score": [0.76],
            "avg_conviction_score": [82],
            "avg_position_health_score": [79],
            "portfolio_equity": [1.10],
            "benchmark_equity": [1.05],
            "alpha_equity": [0.05],
        }
    ).to_csv(review_dir / "portfolio_evolution.csv", index=False)
    pd.DataFrame({"date": ["2024-03-31"], "ticker": ["AAA"], "action": ["HOLD"], "reason": ["Thesis intact"]}).to_csv(
        review_dir / "portfolio_transactions.csv", index=False
    )
    pd.DataFrame({"date": ["2024-03-31"], "ticker": ["AAA"], "action": ["HOLD"], "thesis_persistence_score": [80]}).to_csv(
        review_dir / "portfolio_decision_log.csv", index=False
    )
    pd.DataFrame({"date": ["2024-03-31"], "ticker": ["AAA"], "months_since_entry": [12], "months_thesis_intact": [10], "thesis_persistence_score": [80]}).to_csv(
        review_dir / "portfolio_monthly_holdings.csv", index=False
    )
    pd.DataFrame({"date": ["2024-03-31"], "monthly_turnover": [0.10], "annual_turnover": [0.10], "average_holding_period": [12]}).to_csv(
        review_dir / "portfolio_turnover.csv", index=False
    )
    ...
    expected = [
        ...
        "thesis_history.html",
        "thesis_events.html",
        "thesis_change_report.html",
        "alerts.html",
        "alerts.json",
        "opportunity_cost.html",
        "watchlist.html",
        "position_AAA.html",
        "snapshot_compare_AAA.html",
        "thesis_radar_AAA.html",
    ]
