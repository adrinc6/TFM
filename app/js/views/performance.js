...
(function (global) {
  ...
  // Pestaña Resumen del run: cards del summary + curva + alfa
  async function renderSummary(container, runId, manifest) {
    container.innerHTML = `<div id="perf-charts"></div><h4>Rentabilidad anual</h4><div id="perf-annual"></div>`;
    let data;
  ...
  async function render(container, runId) {
    container.innerHTML = `<div id="perf-charts"></div>
      <h4>Rentabilidad anual</h4><div id="perf-annual"></div>
      <h4>Equity por snapshot</h4><div id="perf-equity"></div>`;
    let data;
    try { data = await api("/api/performance?run_id=" + encodeURIComponent(runId)); }
    catch (e) { container.innerHTML = `<div class="notice">${escapeHtml(e.message)}</div>`; return; }
    const charts = document.getElementById("perf-charts");
